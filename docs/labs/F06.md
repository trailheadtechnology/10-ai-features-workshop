...
::: typescript
   ...
   The starter already does this, near the top of `starter/index.ts`. `DATA` is the `data/` folder two levels up from `index.ts`, `JSON.parse` turns the file's text into an array, and the `Trail` type names the fields so you read one with `t.name`:
...
::: typescript
   ...
   The starter already does this, right below the `trails` line. `process.argv.slice(2)` holds the words after `npm run starter --`, and `find` returns the first trail that matches, or `undefined`:
