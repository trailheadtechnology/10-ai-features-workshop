...
::: typescript
   ...
   The starter already does this, near the top of `index.ts`. `process.argv.slice(2)` holds the words typed after `npm run starter --`; when there are none, the joined string is empty and `||` falls back to the default:
