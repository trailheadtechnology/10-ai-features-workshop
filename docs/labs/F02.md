...
:::
::: python
   `complete/`'s `show()` has the eight labelled lines; it formats the two floats so `3400.0` prints as `3400 ft`, matching `expected-output.md`.
:::
::: typescript
...
:::
::: python
   Below, copied from `complete/`, are the `Verdict` dataclass with its `ok`/`fail` helpers, the non-empty rule, the date rule, and the range rule. `DATE_FORMATS` is the format list above as `strptime` codes, and `{value:g}` is what turns `3400.0` into the `3400 ft` the PASS/REJECT line prints. The grounding rule is in `complete/` too.
