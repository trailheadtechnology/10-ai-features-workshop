...
3. Reuse the embedding search from module 04 against the chunks. Retrieval is literally the previous module. Show the score table for the Sperry question, then run `--top-k 8 --alpha 1.0` and point at ranks 4 through 8: Acadia and Yosemite campfire sections, five of eight chunks from the wrong park.
...
- **Steps:**
  ...
  6. Run question #1 twenty times, not once. A wrong answer that shows up one run in five is invisible in a single run and is the only defect in this module that could hurt somebody.
