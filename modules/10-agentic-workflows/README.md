...
Block 4 (Doing) · Runs on Azure OpenAI / AI Foundry · Double-length slot (about 60 minutes)
...
This is the capstone because it composes the day. The agent searches trails semantically (04), grounds itself in current condition reports (08's data), works with structured tool results (02's lesson in reverse), and pauses for human confirmation before filing the permit (09's policy, applied). It's also the module where model choice stops being negotiable. A dropped or malformed tool call doesn't degrade the experience, it breaks the loop, so this module runs on Azure OpenAI rather than gambling the finale on a local model. And because an agent acts instead of answering, guardrails are part of the design, not an add-on: a step budget so it can't loop forever, and a confirmation gate before anything irreversible.
...
- **Rough cost & effort:** the highest of the ten. Weeks to months, frontier-model API costs, and real design work on guardrails, confirmation gates, and observability. Do a module 1-9 feature first; do this when the goal-shaped problem is worth it.
