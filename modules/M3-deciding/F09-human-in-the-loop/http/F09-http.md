...
### Lab Step 0: Requests 1 to 4, Read Them as an Editor

Send requests 1 to 3 and read the drafts: what would you change before this went out under your name? Then send request 4, the overdue-hiker voicemail. The system prompt tells the model to output `ESCALATE:` for emergencies. Watch what it does instead. That is requests 1 to 4; request 5, the "repair" with the rule moved to the top of the prompt, is the stretch goal.

Check: three usable drafts, then a warm reassuring note to a woman whose husband is four hours overdue (recorded 3/3). A prompt instruction is a request.

### Lab Step 1: The Policy Worksheet
...
Send request 5, the "repair" with the escalation rule moved to the top of the prompt. Check: `ESCALATE` followed by the reassuring note anyway (recorded 3/3). Moving the instruction does not make it a gate.
