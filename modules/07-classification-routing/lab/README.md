...
- `reference-labels.json`: the category for each id from the taxonomy the module uses (`permit | conditions | complaint | lost-and-found | emergency | general`), the queue each category routes to, and notes on the two emergencies and on the second label that is defensible for `inq-0035`.
- `ollama.http`: the classify request against Ollama (`llama3.2`), shown on three inlined messages: a routine conditions question, an emergency, and the ambiguous one. The taxonomy is plain-language category descriptions in the prompt, and the `format` field is a JSON schema whose enum is the six categories, so the model cannot invent a label. Copy request 1 and swap the message text to work through the rest of the slice.
