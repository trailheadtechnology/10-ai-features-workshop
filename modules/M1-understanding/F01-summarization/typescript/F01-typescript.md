# TypeScript Demo for 01 Summarization

Two scripts, both reading the trip reports in [`../data/`](../data/):

- `starter/index.ts`: the demo's starting point. One `OpenAI` client pointed at Ollama, one call, the naive "Summarize this trip report" prompt. Run it on `tr-0001.md` and read the book report.
- `complete/index.ts`: the finished demo as shown on stage. Same call, three prompts selected by flag: the naive one, the 3-bullet hiker briefing with the grounding lines, and the one-line headline for a card UI. `--audience ranger` swaps who the briefing is for.

Setup once, then run from this directory (`tsx` runs the `.ts` files directly, no build step):

```bash
npm install
npm run starter
npm run complete                                # naive prompt on the buried-hazard report
...
The steps in [`../F01-lab.md`](../F01-lab.md), done in TypeScript: start from `starter/index.ts` and end where `complete/index.ts` is. Edit the starter in place (or copy it first); `complete/` is the answer key, and its comments say why each piece is there. Run with `npm run starter` from the `typescript/` directory; the flags shown for later steps are the ones `complete/` supports, so add the same argument parsing or hard-code the value.
