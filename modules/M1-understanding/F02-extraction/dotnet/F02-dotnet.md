...
Here is how it works, in `starter/Program.cs`. Line one is the path (the first argument, or `tr-0007.md`), line two reads the file and drops the front matter, and the call sends the naive prompt and prints the raw reply text:
...
1. Review how the existing code from the starter project reads `data/tr-0007.md` and strips the front matter (split on `---`, keep the third part, trimmed).
   ...
   The read is near the top of `starter/Program.cs`:
...
1. Review how the existing code from the starter project makes the report path a command-line argument, defaulting to `tr-0007.md`.
   ...
   `args` holds whatever you type after `dotnet run --`:
