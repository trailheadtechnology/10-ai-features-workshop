...
1. Review how the existing code from the starter project reads `data/tr-0007.md` and strips the front matter (split on `---`, keep the third part, trimmed).
   ...
   `stripFrontMatter` is the function near the top of `starter/index.ts`. Splitting on `---` gives the empty text before the front matter, the front matter, and then the report; `parts.slice(2).join("---")` glues everything from the third part on back together, so a `---` inside the report body survives:
...
6. Print a heading with the file name, then, under a `-- what the model gave us --` line, the eight fields labelled `trail:`, `park:`, `date:`, `distance: N mi`, `elev gain: N ft`, `wildlife: [...]`, `conditions: [...]`, `hazards: [...]`, as in the complete-demo block of `expected-output.md`. Print `null` for a null.
   ...
   A `show()` function gives you the eight labelled lines; `${value ?? "null"}` prints `null` for a null and `basename(reportPath)` gives the `== tr-0007.md ==` heading.
...
1. Review how the existing code from the starter project makes the report path a command-line argument, defaulting to `tr-0007.md`.
   ...
   `process.argv[2]` is the first thing you type after `npm run starter --` (the first two entries are Node and the script), it is `undefined` when you type nothing, and `??` then falls back to `tr-0007.md` in `DATA`, the feature's `data/` folder:
