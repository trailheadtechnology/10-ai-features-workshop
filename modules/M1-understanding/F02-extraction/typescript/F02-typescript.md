# TypeScript Demo for 02 Extraction

Two scripts, both reading from [`../data/`](../data/):

- `starter/index.ts`: the naive approach: ask for JSON in the prompt and print whatever comes back, preamble, markdown fences, drifting field names and all.
- `complete/index.ts`: the finished demo as shown on stage. A zod `TripFacts` schema with nullable fields and per-field descriptions, handed to `.parse()` so the reply comes back typed. Then the validator: grounding checks for names, strict date parsing, range and zero checks for the numbers, and rejected fields coerced to `null` before anything would be stored.
...
const TripFacts = z.object({
  ...
  conditions: z.array(z.string()).describe("Short phrases describing trail conditions the report mentions. Empty array if none."),
...
function inRange(field: string, value: number | null, max: number, unit: string): Verdict {
  ...
  if (value === 0) return fail(field, `${value} ${unit}`, "0 is not a measurement; the report gave no figure, so this should be null");
  return value > max ? fail(field, `${value} ${unit}`, `implausible: over ${max} ${unit}`) : pass(field, `${value} ${unit}`);
}

function validDate(field: string, value: string | null): Verdict {
  if (value === null) return pass(field, null);
  const parsed = parseDate(value);            // explicit formats only; see complete/ for parseDate
  return parsed
    ? pass(field, value, `${parsed.y}-${String(parsed.m).padStart(2, "0")}-${String(parsed.d).padStart(2, "0")}`)
    : fail(field, value, "does not parse as a date; no parser can store this");
}
```
