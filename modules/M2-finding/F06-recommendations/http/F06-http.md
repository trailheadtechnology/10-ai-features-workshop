...
### Lab Steps 2 to 4: Vectors, Then Nearest Neighbors

Feature 04's cached vectors will not work here: `data/trails.json` is a different 30-trail slice from the same 200-trail catalog (the two share 7 trails). `data/trail-embeddings.json` has all 30 vectors precomputed for this slice, or request 2 shows how to embed a batch fresh. Take target trail `trail-0117`'s vector, cosine-score every other trail against it, skip the target, print the top 5.

Check: Gunsight Lake Approach at 0.7849 on top. Read the difficulty column against the target's.

### Lab Step 5: The Other Targets
