...
2. Review how the existing code from the starter project parses it into a list of trail objects.
   ...
   The starter reads `"../../data/trails-slice.json"` relative to the working directory, which is why you run from `starter/`, and deserializes it into the `Trail` record with a snake_case naming policy (so JSON `distance_mi` fills the C# property `DistanceMi`). It happens near the top of `Program.cs`:
...
1. Review how the existing code from the starter project takes the query from the command-line arguments, joined with spaces. It defaults to this when there are none:
   ...
   It happens at the top of `Program.cs`. `args` holds the words typed after `dotnet run --`:
