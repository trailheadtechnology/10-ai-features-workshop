...
That is the disaster in the module intro, and it does not require the model to be stupid. The draft is well written. It is well written and it is a lie, and the model had explicit instructions not to write it.
...
**The lesson, and it is the whole module in one line:** a prompt instruction is a request, and a policy lane is a guarantee. Emergencies must never reach the model. `dotnet/complete` enforces this in code, before the API call:
