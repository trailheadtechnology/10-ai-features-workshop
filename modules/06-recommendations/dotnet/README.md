...
Both run against Ollama (`nomic-embed-text`), matching the demo outline in [../README.md](../README.md). From `complete/`:
...
`Cosine` is a dozen lines at the bottom of `complete/Program.cs`, and the recommendation itself is the LINQ query above it. That is the demo's point: this is the module 04 search code with the query text swapped for an item's vector.
