...
- **.NET**: install .NET 10 or newer ([dot.net](https://dot.net)) and confirm `dotnet --version` agrees. `global.json` pins the SDK to 10.0.x, so a machine with only .NET 9 fails at the first build with an error about that file. `dotnet build workshop.slnx` at the repo root builds all twenty projects once, so the first `dotnet run` in the room is not a compile wait.
